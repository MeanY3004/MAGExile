...
// == set "UPSMON_grpid" to units in the group and EH===============================  
{
    ...
    {//soldiers 
        _x addMPEventHandler ["hit", {nul = _this spawn R_SN_EHHIT}];	
        sleep 0.05;
        _x addMPEventHandler ["killed", {nul = _this spawn R_SN_EHKILLED}];	
        sleep 0.05;
    };
} foreach units _npc;
...
if (isnil {_npc getVariable ("UPSMON_grpid")}) then {		
    ...
    {//soldiers
        _npc addMPEventHandler ["hit", {nul = _this spawn R_SN_EHHIT}];	
        sleep 0.05;
        _npc addMPEventHandler ["killed", {nul = _this spawn R_SN_EHKILLED}];	
        sleep 0.05;
    }
    else
    { //civilian
        ...
        sleep 0.05;
        _npc addMPEventHandler ["firedNear", {nul = _this spawn R_SN_EHFIREDNEAR}];
        sleep 0.05;	
        
        _npc removeAllEventHandlers "killed";
        sleep 0.05;
        _npc addMPEventHandler ["killed", {nul = _this spawn R_SN_EHKILLEDCIV}];
        sleep 0.05;
    }
};
...
if (_deletedead>0) then {
    {
        _x addMPEventHandler['killed',format["[_this select 0,%1] spawn KRON_deleteDead",_deletedead]];
        sleep 0.01;
    }forEach _members;
};
...
//		if (!_nowp && {alive _npc} && {canmove _npc} && {_wptype == "MOVE"} && {_timeontarget >= 60}) then {
    ...
    if (isNil ("_cont")) then { // exit loop criterium
		_cont = 0;
        _exit=true;
    };
    ...
    if(_allow_respawn) then {           // start the respawn logic
       ...
       //[nil, _x, "per", rHideObject, true] call RE; 
        ...
        if(_is_veh_group) then {
			...
			[[[_leader],"SAR_AI_trace_veh"],"bis_fnc_spawn",true,true] call BIS_fnc_MP;
			[[[_leader],"setIdentity 'id_SAR_sold_lead'"],"bis_fnc_spawn",true,true] call BIS_fnc_MP;
			[[[_leader],"SAR_AI_reammo"],"bis_fnc_spawn",true,true] call BIS_fnc_MP;
        } else {
            //_leader setVehicleInit "[this] spawn SAR_AI_trace;this setIdentity 'id_SAR_sold_lead';[this] spawn SAR_AI_reammo;";
			[[[_leader],"SAR_AI_trace"],"bis_fnc_spawn",true,true] call BIS_fnc_MP;
			[[[_leader],"setIdentity 'id_SAR_sold_lead'"],"bis_fnc_spawn",true,true] call BIS_fnc_MP;
			[[[_leader],"SAR_AI_reammo"],"bis_fnc_spawn",true,true] call BIS_fnc_MP;
        };
        
        _leader addMPEventHandler ["HandleDamage",{if (_this select 1!="") then {_unit=_this select 0;damage _unit+((_this select 2)-damage _unit)*SAR_leader_health_factor}}];        
        ...
        _i=0;
        {
            ...
            if (_i>1) then {
                ...
                if(_is_veh_group) then {
					...
					[[[_newunit],"SAR_AI_trace_veh"],"bis_fnc_spawn",true,true] call BIS_fnc_MP;
					[[[_newunit],"setIdentity 'id_SAR'"],"bis_fnc_spawn",true,true] call BIS_fnc_MP;
					[[[_newunit],"SAR_AI_reammo"],"bis_fnc_spawn",true,true] call BIS_fnc_MP;					
                } else {
                    //_newunit setVehicleInit "[this] spawn SAR_AI_trace;this setIdentity 'id_SAR';[this] spawn SAR_AI_reammo;";
					[[[_newunit],"SAR_AI_trace"],"bis_fnc_spawn",true,true] call BIS_fnc_MP;
					[[[_newunit],"setIdentity 'id_SAR'"],"bis_fnc_spawn",true,true] call BIS_fnc_MP;
					[[[_newunit],"SAR_AI_reammo"],"bis_fnc_spawn",true,true] call BIS_fnc_MP;					
                };
                ...
                _newunit addMPEventHandler ["HandleDamage",{if (_this select 1!="") then {_unit=_this select 0;damage _unit+((_this select 2)-damage _unit)*1}}];    
        ...
        {		
            ...
            if(_iscar) then {
                ...
                if(SAR_heli_shield) then {
                    _veh addMPEventHandler ["HandleDamage", {returnvalue = _this spawn SAR_AI_VEH_HIT;}];  
                }else {
                    _veh addMPEventHandler ["HandleDamage", {_this spawn SAR_AI_VEH_HIT;_this select 2;}];  
                };
            };
		...
		[[[_UCthis],"SAR_AI_UPSMON"],"bis_fnc_spawn",true,true] call BIS_fnc_MP;
        //sleep 0.1;
        //processInitCommands;
    };
};	
