...
_leader setIdentity "id_SAR_sold_man";
...
[_man2heli] spawn SAR_AI_trace;
_man2heli setIdentity "id_SAR_surv_lead";
...
[_man3heli] spawn SAR_AI_trace;
_man3heli setIdentity "id_SAR_band";
			...
			if(_hcID select [0,2] isEqualTo 'HC')then {
				...
				if (_SAIS_HC) then {
					if (SAR_DEBUG) then {
						...
						diag_log format ["Sarge's AI System: Moving group %1 to Headless Client has failed",_groupheli];
					};
				};
			};
		} forEach allPlayers;
