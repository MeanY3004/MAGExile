...
if((!isNull _aikiller) && {(_playerUID != "") && {_aikiller isKindOf "Exile_Unit_Player"}}) then {
    ...
    if (_aikilled_group_side == SAR_AI_friendly_side) then {
        if(SAR_DEBUG)then{diag_log format["SAR_DEBUG: Adjusting respect for survivor or soldier kill by %2 for %1",_aikiller,SAR_surv_kill_value];};
    ...
    if (_aikilled_group_side == SAR_AI_unfriendly_side) then {
        if(SAR_DEBUG)then{diag_log format["SAR_DEBUG: Adjusting respect for bandit kill by %2 for %1",_aikiller,SAR_band_kill_value];};
