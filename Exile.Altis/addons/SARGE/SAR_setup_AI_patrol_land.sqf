...
if (_argc > 1) then {
    ...
    switch (_grouptype) do
    {
        case 1:
        {
            ...
            _initstring1 = "addons\SARGE\SAR_trace_from_vehicle.sqf";
            _initstring2 = "id_SAR_sold_man";
            _initstring3 = "addons\SARGE\SAR_reammo_refuel_AI.sqf";
...
if (_ai_type == "") exitWith {diag_log format ["Sarge's AI System: Group has no type. Exiting spawn script for group type %1",_grouptype];};
...
// create the vehicle and assign crew
{
    ...
    if((_veh_setup select 0) == 1) then {
		...
		[[[_leader],_initstring1],"BIS_fnc_execVM",true,false] call BIS_fnc_MP;
		[[_leader,_initstring2],"setIdentity",false,false] call BIS_fnc_MP;
		[[[_leader],_initstring3],"BIS_fnc_execVM",false,false] call BIS_fnc_MP;
    ...
    for "_i" from 0 to (_snipers - 1) do
    {
		...
		[[[_this],"addons\SARGE\SAR_trace_from_vehicle.sqf"],"BIS_fnc_execVM",true,false] call BIS_fnc_MP;
		[[_this,"id_SAR"],"setIdentity",false,false] call BIS_fnc_MP;
    ...
    for "_i" from 0 to (_riflemen - 1) do
    {
		...
		[[[_this],"addons\SARGE\SAR_trace_from_vehicle.sqf"],"BIS_fnc_execVM",true,false] call BIS_fnc_MP;
		[[_this,"id_SAR_sold_man"],"setIdentity",false,false] call BIS_fnc_MP;
			...
			if(_hcID select [0,2] isEqualTo 'HC')then {
				...
				if (_SAIS_HC) then {
					if (SAR_DEBUG) then {
						...
						diag_log format ["Sarge's AI System: Moving group %1 to Headless Client has failed",_groupvehicles];
					};
				};
			};
		} forEach allPlayers;
