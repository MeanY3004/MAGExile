...
if (SAR_dynamic_spawning) then {
	switch (_worldname) do {
       case "altis":
        {
            #include "map_config\SAR_cfg_grid_altis.sqf"
        };
		case "chernarus":
        {
	...
	diag_log format["Sarge's AI System: Now generating map grid for %1.",_worldname];
...
switch (_worldname) do {
    case "altis":
    {
        #include "map_config\SAR_cfg_grps_altis.sqf"
    };
	case "chernarus":
    {
