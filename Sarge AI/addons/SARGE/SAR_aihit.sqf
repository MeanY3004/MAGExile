...
switch (elec_hc_connected) do {
	...
	case 1:
	{
		if (SAR_HITKILL_DEBUG && !hasInterface) then {
			diag_log format["SAR_HITKILL_DEBUG: AI hit - %2 - Type: %1 Side: %3 Group Side: %4",_aikilled_type,_ai,_aikilled_side,_aikilled_group_side];
			diag_log format["SAR_HITKILL_DEBUG: AI attacker - Type: %1 Name: %2 Side: %3 Group Side: %4",_aikiller_type,_aikiller_name, _aikiller_side,_aikiller_group_side];
		};
	};
...
if((!isNull _aikiller) && {(_playerUID != "") && {_aikiller isKindOf "Exile_Unit_Player"}}) then {
    ...
    if (_aikilled_group_side == SAR_AI_friendly_side) then { // hit a friendly AI
        ...
        switch (elec_hc_connected) do {
			case 0: {if(SAR_HITKILL_DEBUG && isServer)then{diag_log format["SAR_HITKILL_DEBUG: friendly AI was hit by Player %1",_aikiller];};};
			case 1: {if(SAR_HITKILL_DEBUG && !hasInterface)then{diag_log format["SAR_HITKILL_DEBUG: friendly AI was hit by Player %1",_aikiller];};};
		};
        ...
        switch (elec_hc_connected) do {
			case 0: {if(SAR_HITKILL_DEBUG && isServer)then{diag_log format["SAR_HITKILL_DEBUG: Adjusting respect for survivor hit by %2 for %1",_aikiller,(SAR_surv_kill_value/10)];};};
			case 1: {if(SAR_HITKILL_DEBUG && !hasInterface)then{diag_log format["SAR_HITKILL_DEBUG: Adjusting respect for survivor hit by %2 for %1",_aikiller,(SAR_surv_kill_value/10)];};};
		};

        if((rating _aikiller > -10000) && !isServer) then { //check if shooter is not already marked as enemy
            switch (elec_hc_connected) do {
				case 0: {if(SAR_HITKILL_DEBUG && isServer)then{diag_log format["SAR_HITKILL_DEBUG: Marking Player %1 as an enemy for a friendly AI hit!",_aikiller];};};
				case 1: {if(SAR_HITKILL_DEBUG && isServer)then{diag_log format["SAR_HITKILL_DEBUG: Marking Player %1 as an enemy for a friendly AI hit!",_aikiller];};};
			};
    ...
    if (_aikilled_group_side == SAR_AI_unfriendly_side) then { // hit an unfriendly AI
		...
		switch (elec_hc_connected) do {
			case 0: {if(SAR_HITKILL_DEBUG && isServer)then{diag_log format["SAR_HITKILL_DEBUG: unfriendly AI was hit by Player %1",_aikiller];};};
			case 1: {if(SAR_HITKILL_DEBUG && !hasInterface)then{diag_log format["SAR_HITKILL_DEBUG: unfriendly AI was hit by Player %1",_aikiller];};};
		};
		...
		switch (elec_hc_connected) do {
			case 0: {if(SAR_HITKILL_DEBUG && isServer)then{diag_log format["SAR_HITKILL_DEBUG: Adjusting respect for bandit hit by %2 for %1",_aikiller,(SAR_band_kill_value/10)];};};
			case 1: {if(SAR_HITKILL_DEBUG && !hasInterface)then{diag_log format["SAR_HITKILL_DEBUG: Adjusting respect for bandit hit by %2 for %1",_aikiller,(SAR_band_kill_value/10)];};};
		};
    };
};
